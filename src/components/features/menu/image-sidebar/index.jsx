...
export default function ImageSidebar({ resId }) {
    ...
    const fetchImages = async (query, pageNum = 1) => {
        ...
        try {
            ...
            const res = await axios.get(`/api/images/search?q=${encodeURIComponent(query)}&page=${pageNum}&limit=12`);
            ...
            if (res.data.success) {
                if (pageNum === 1) {
                    ...
                    setImages((prev) => {
                        ...
                        const newImages = res.data.data.filter(img => !existingIds.has(img._id));
    ...
    const handleScroll = (e) => {
        ...
        if (scrollHeight - scrollTop <= clientHeight * 1.5 && hasMore && !loading && !isLoadingMore) {
